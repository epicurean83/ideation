...
**Fable 5 главным оркестратором не ставить.** Только по явному запросу пользователя и только на многодневные прогоны; в deepdive его цена не окупается ни на одной фазе.
...
Уровни `xhigh` и `max` появились после первой редакции этого файла. `xhigh` — рабочий максимум для кодинга и агентных задач; `max` прожорлив и склонен к overthinking. Для deepdive: `xhigh` уместен на Phase 6 (red team) и Phase 1 (reframing), `max` — не уместен нигде (ресёрч упирается в качество источников, не в глубину рассуждения одной модели).
